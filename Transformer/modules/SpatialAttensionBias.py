...
class SpatialAttensionBias(nn.Module):
    def __init__(self,graph:GraphFeatures,d,S,hno,max_dist=50):
        ...
        self.graph = graph
        self.d = d
        self.sp_enc = nn.Embedding(S+1,hno,padding_idx=0)
    def forward(self,x):
        B,T,N,D = x.shape
        attn_bias = torch.zeros([B,N+1,N+1],dtype = torch.float,device=x.device)
        graph_attn_bias = attn_bias.clone().unsqueeze(1).repeat(1,self.hno,1,1)
        sp_bias = self.graph.spd.long().unsqueeze(0).repeat(B,1,1).to(x.device)
